...
1. **PE subsystem patch (GUI → console).** A build-time Python patcher
   ([`content/tribes_dual_patcher.py`](../content/tribes_dual_patcher.py)) flips the PE header's
...
without escape-stripping). See [`Services/TerminalSession.cs`](../panel/Services/TerminalSession.cs).
...
- **New VC++ 2022 runtime** — real Microsoft DLLs (`msvcp140`, `vcruntime140`, `concrt140`, …)
  are downloaded (`VCRUN22_URL`, a `vcrun22.zip` of `*.dll_x86`), renamed to `.dll`, and dropped
  into the Wine `system32`, with `WINEDLLOVERRIDES` set to prefer the native copies.
...
- Source: [`Services/GameSupervisor.cs`](../panel/Services/GameSupervisor.cs) ·
  [`content/tribes_dual_patcher.py`](../content/tribes_dual_patcher.py)
